...
<script>
import MangaSearchCards from "./MangaSearchCards.vue";
import Loading from "../Loading.vue";

export default {
  name: "MangaSearch",
  props: [],
  components: { MangaSearchCards, Loading },
  data() {
    return {
      searchedData: [],
      white: "white",
    ...
    goSearch: async function () {
      ...
      try {
        const response = await fetch(
          `https://api.jikan.moe/v3/search/manga?q=${searchParams}&limit=25`
        );
        ...
        this.searchResultsShownComponent = "MangaSearchCards";
      } catch (error) {
        console.log(error);
